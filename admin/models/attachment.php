<?php
// No direct access to this file
defined('_JEXEC') or die('Restricted access');
 
// Include dependancy of the main model form
jimport('joomla.application.component.modelform');
// Include dependancy of the dispatcher
jimport('joomla.event.dispatcher');
 
class VjeecDcmModelAttachment extends JModelList
{
    public function addAttachment($file, $uploaded=false)
    {
        jimport('joomla.filesystem.file');
        $ret = new stdClass();
        
        $config         = JFactory::getConfig();
        $tmp_dir       = $config->get('tmp_path');
        if (!$uploaded) {
            //Clean up filename to get rid of strange characters like spaces etc
            $tmpUpld = JFile::makeSafe($file['name']);
            //$filename = $attId . '_' . '01.' . JFile::getExt($filename);
            $src = $file['tmp_name'];
            $tmpUpld = $tmp_dir . '/' . $tmpUpld;
            $uploadResult = JFile::upload($src, $tmpUpld);
            
            if (!$uploadResult) { 
                return null;
            }
        } else {
            $tmpUpld = $tmp_dir. '/' . $file;
        }
        
        if (!file_exists($tmpUpld)) { 
        	$ret->error = 1;
        	$ret->msg = 'File not found '.$tmpUpld;
        	return $ret;
        }
        
        $userId = JFactory::getUser()->id;
        $db = JFactory::getDBO();
        
        $attachment = new stdClass();
        $attachment->owner_id = JFactory::getUser()->id;
        $result = $db->insertObject('#__vjeecdcm_attachment', $attachment, 'id');
        // Add observer here
        if (!$result) {
        	JFile::delete($tmpUpld);
            return null;
        }
        
        $attId = $db->insertid();
        $vjeec_dir = $tmp_dir . '/../vjeecdcm';
        // Upload the electronic version
        //Import filesystem libraries. Perhaps not necessary, but does not hurt
        $file_name = $attId . '_01.' . JFile::getExt($tmpUpld);
        $dest = $vjeec_dir . '/' . $file_name;
        
        $move = JFile::move($tmpUpld, $dest);
        if (!$move) { 
        	$ret->error = 1;
        	$ret->msg = 'Cannot move file from '.$tmpUpld .' to '. $dest;
        	return $ret;
        }
        
        $attFileObj = new stdClass();
        $attFileObj->path = 'vjeecdcm/'. $file_name;
        $attFileObj->attachment_id = $attId;
        
        $result = $db->insertObject('#__vjeecdcm_attachment_file', $attFileObj, 'id');
        if ($result) {
            return $attId;
        } else {
            return null;
        }
    }
    
    public function removeAttachment($attachmentId) {
        $path = $this->getFilePath($attachmentId);
        if ($path != NULL) {
            $trash = $path . '.trashed';
            JFile::move($path, $trash);    
        }
        $db = JFactory::getDbo();
        $query = $db->getQuery(true);
        $query->delete('#__vjeecdcm_attachemnt')
              ->where(array('id = ' . $attachmentId));
        $db->setQuery($query);
        $result = $db->query();
        
    }
    
    public function getFilePath($attachmentId)
    {
        $db = JFactory::getDBO();
        $query = $db->getQuery(true);
        $query->select(array('id','attachment_id', 'path'))
            ->from('#__vjeecdcm_attachment_file')
            ->where('attachment_id = '. $attachmentId);
        try {
            $db->setQuery($query);
            $result = $db->loadObject();
            if (isset($result->path)) {
                return $result->path;
            }
            return null;
        } catch (Exception $e) {
            return null;
        }
    }
    
    public function getFileInfo($attachmentId)
    {
        $db = JFactory::getDBO();
        $query = $db->getQuery(true);
        $query->select(array('path', 'type'))
            ->from('#__vjeecdcm_attachment_file')
            ->where('attachment_id = '. $attachmentId);
        try {
            $db->setQuery($query);
            $result = $db->loadObjectList();
            if ($result == null) return null;
            return $result[0];
        } catch (Exception $e) {
            return null;
        }
    }
    
}
