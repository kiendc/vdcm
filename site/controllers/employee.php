...
class VjeecDcmControllerEmployee extends JControllerForm
{
    ...
    public function requestViewFormHandler()
    {
        ...
        ////dump($_POST['check'], 'request view handler');
        ...
        //$redirectURL = 'index.php?Itemid=1225&&dirId=' . $dirId;
        if(!empty($_POST['check']))
        {
            ...
            {
                case 'moveSel':
                    //dump($_POST['directorySelected'], 'Move requests to a directory');
                ...
                case 'updateSel':
                    ...
                    {
                        //dump($_POST['check'], 'Request handler, selected requests');
                        //dump($_POST['steps'], 'Request handler, selected requests');  
    ...
    public function requestCorrectionFormHandler()
    {
        ...
        if ($_POST['buttonClicked'] == 'deleteSel')
        {
            ...
            if(!empty($_POST['check']))
            {
                ...
                //{
                //    //dump($check, 'Controller::schoolRequestViewFormHandler, check'); 
                //}
                $model = $this->getModel('request');
        ...
        elseif ($_POST['buttonClicked'] == 'updateSel')
        {
            if(!empty($_POST['check']))
            {
                ////dump($_POST, 'correction form handler, post');
                $model = $this->getModel('request');
    ...
    public function addSchoolFormHandler()
    {
        ...
        $this->setRedirect(JRoute::_('index.php?option=com_vjeecdcm&view=emplschool' ));
    }

    public function certGenFormHandler()
    {
        ////dump($_POST, "certGenFormHandler, post");
        ...
        ////dump($output, 'Generating certificate, output');
        jexit();
    }
    ...
    public function getSelect2SchoolList()
    {
        ...
        ////dump($arrSchools, 'Select2 list of schools');
        echo json_encode($arrSchools);
        jexit();
    }
    ...
    public function modifyTargetSchool()
    {
        ...
        if ($result != NULL)
        {
            ////dump($result, 'modify target school result');
    ...
    {     
        ////dump($_POST, 'edit request detail, post');
        $model = $this->getModel('request');
        $reqId = intval($_POST['pk']);
        $result = new stdClass();
        switch ($_POST['name']) {
    ...
    public function editDiplomaDetail()
    {
        ...
        ////dump($_POST, 'edit diploma detail, post');
        $result = new stdClass();
        switch ($_POST['name']) {
    ...
    public function editDiplomaHolderInfo()
    {
        ...
        ////dump($_POST, 'edit diploma holder info, post');
    ...
    public function prepareRequestDataForEmployees(&$requests)
    {
        ...
        foreach ($requests as $rq)
        {
            ...
            foreach ($rq->holder->info as $item)
            {
    ...
    {
        ////dump($_POST, 'employee.getJSONRequest POST');
        ...
        ////dump($requests[0], 'requests ');
        //$requests = $model->getObservingRequests(604); 
        //$nbRows = count($requests);
    ...
    public function getRequestDetail()
    {
        ...
        if ($rq != NULL)
        {
            ...
            ////dump($rq, 'get request detail, request');
        }
    ...
    public function getJSONDiplomas()
    {
        ////dump($_POST, 'employee.getJSONRequest POST');
        $offset = intval($_POST['start']);
        $draw = intval($_POST['draw']);
        ...
        $model = $this->getModel('diploma');
        $diplomas = $model->getAllDiplomas($offset, $limit, $searchValue);
        $this->prepareDiplomaDataForEmployees($diplomass);
        ////dump($requests[0], 'requests ');
        //$requests = $model->getObservingRequests(604); 
        //$nbRows = count($requests);
        //$nbRows = $model->getNumberObservingRequests($userId);
